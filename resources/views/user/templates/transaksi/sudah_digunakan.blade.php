    ...
    <div class="card" style="height:510px;">
      <!-- Card image -->
        <div class="view overlay">
          ...
          <div class="card-body">
          <!-- Title -->
          <?php $namaevent = strlen($j->event->nama_event);
            if ($namaevent > 14) {
                $nevent = substr($j->event->nama_event, 0, 14).'...';
            } else {
                $nevent = $j->event->nama_event;
            } ?>
            <h5 class="card-title text-center" data-toggle="tooltip" data-placement="top" title="{{$j->event->nama_event}}"><?= $nevent; ?>
            </h5>
          <hr>
          <!-- Text -->
          <p class="card-text">
              ...
              <tr>
                  ...
                  <td class="px-2">:</td>
                  <td class="text-left text-muted"><i class="fas fa-qrcode mr-2"></i>Sudah digunakan</td>
              </tr>
              <tr>
                  <td class="text-left">Tanggal Beli</td>
                  <td class="px-2">:</td>
                  <td class="text-left"></i>{{ date("d F Y", strtotime($j->updated_at )) }}</td>
              </tr>
              <tr>
                  <td class="text-left">Jumlah Tiket</td>
                  <td class="px-2">:</td>
                  <td class="text-left">Rp. {{ number_format($j->total,0,',','.')}}</td>
              </tr>
          </table>
          <hr/>
          ...
          <a class="text-center text-dark" data-toggle = "modal" data-target = "#details"><i class="fas fa-eye mr-2"></i>Detail</a>
          </div>
    </div>
        ...
        <div class = "modal-body text-center">
            <div class="row">
                <div class="col-md-6">
                    ...
                    <tr>
                        <th colspan="4" class="text-left">ID Transaksi : {{ $j->id_transaksi}}
                        </th>
                    </tr>
                    <tr>
                        <th width="50px">
                        <img class="card-img-top img img-responsive full-width" src="{{ asset('storage/event/'.$j->event->banner_event.'') }}" alt="Card image cap" >
                        </th>
                        <th colspan=3>{{ $j->event->nama_event }}</th>
                    </tr>
                    <tr>
                  ...
                  @endforeach
                    <tr>
                        ...
                        <td><input type="text"value="{{ $j->nover }}" size="5" readonly name="nover" id="nover" class="text-right inputs px-n3"></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td></td>
                        <td class="mx-n2">Rp.</td>
                        <td><input type="text" class=" h5 inputs text-warning text-right font-weight-bold px-n3" name="total_jumlah" id="total_jumlah" size="5" value="{{ number_format($j->total,0,',','.')}}" readonly></td>
                    </tr>
                </table>
                </div>
                <div class="col-md-6">
